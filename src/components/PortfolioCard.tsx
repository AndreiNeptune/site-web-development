...
export default function PortfolioCard({ item, index, isLarge = false }: PortfolioCardProps) {
  return (
    <motion.div
      ...
      data-portfolio-card
    >
      <Link href={`/portofoliu/${item.slug}`} className="block relative overflow-hidden rounded-3xl bg-[#111] p-4 sm:p-8 md:p-12 border border-white/5 hover:border-white/10 transition-colors">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 px-2">
        <div>
          <Link href={`/portofoliu/${item.slug}`} className="inline-flex items-center gap-2 group/title">
            <h3 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight group-hover/title:text-blue-400 transition-colors">
              {item.title}
            </h3>
          ...
          <div className="flex items-center gap-3 mt-2 text-zinc-400 text-sm sm:text-base font-medium">
        ...
        <Link
          ...
          className="hidden sm:inline-flex items-center justify-center h-12 px-6 rounded-full bg-white text-black font-semibold hover:bg-blue-50 hover:text-blue-600 transition-colors"
        >
          Case Study
        </Link>
      </div>
